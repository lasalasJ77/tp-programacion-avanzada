...
public class AlumnoSQLDAOImpl implements AlumnoDAO {
    ...
    public void modificar(Alumno a) throws Exception {
        PreparedStatement ps = ConexionSQL.getConexion().prepareStatement("UPDATE alumnos SET apellido=?, nombre=? WHERE dni=?");
    ...
    public Alumno consultar(String dni) throws Exception {
        ...
        if (rs.next()) {
            return new Alumno(rs.getString("dni"), rs.getString("apellido"), rs.getString("nombre"), rs.getBoolean("eliminado"));
        }
        return null;
    }
    ...
    public List<Alumno> obtenerTodos(boolean incluirEliminados) throws Exception {
        ...
        while (rs.next()) {
            lista.add(new Alumno(rs.getString("dni"), rs.getString("apellido"), rs.getString("nombre"), rs.getBoolean("eliminado")));
        }
        return lista;
    }
}
